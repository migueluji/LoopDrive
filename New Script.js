...
function init() {
    console.log("init"); signinButton.style.display = 'flex';
...
function handleAuthClick() {
    ...
    // tokenClient.callback = async function tokenCallback(resp) {
...
function handleCredentialResponse(response) {
    ...
    if (response.credential) {
        ...
        document.getElementById("profile").innerHTML = `
        ${user.given_name}
        <img src="${user.picture}" style="width: 32px; height: 32px; border-radius: 50%; object-fit: cover;margin: auto 10px">
        `;
