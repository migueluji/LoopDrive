class DisplayActor extends PIXI.Container {
    ...
    constructor(canvas, actor, cast, game) {
        ...
        var texture = null;
        var existsImage = Boolean(app.loader.resources[actor.image]);
        if (actor.image && existsImage) texture = app.loader.resources[actor.image].texture;
    ...
    createSprite(actor, texture) {
        ...
        this.tilingSprite.alpha = (actor.spriteOn) ? actor.opacity : 0;
        if (actor.sleeping && actor.spriteOn) this.tilingSprite.alpha = 0.5;
        this.tilingSprite.tint = PIXI.utils.string2hex(actor.color);
        ...
        this.tilingSprite.scale.x = (actor.flipX) ? -actor.scaleX : actor.scaleX;
        this.tilingSprite.scale.y = (actor.flipY) ? actor.scaleY : -actor.scaleY; // axis change and rotate the sprite scale
